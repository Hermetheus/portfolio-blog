  ...
  loadCursorBlink(ref, ttw, end, isLastSentence) {
    ...
    const nextTimeOut = setTimeout(() => {
      if (self.interval) {
        ...
        if (
          self.lastReference &&
          self.lastReference.innerText.substring(
            self.lastReference.innerText.length - 1
          ) === "|"
        ) {
          self.lastReference.innerText = self.lastReference.innerText.substring(
            0,
            self.lastReference.innerText.length - 1
          )
        }
      ...
      if (!isLastSentence) {
        self.interval = setInterval(() => {
          ...
          if (isEnd) {
            if (
              reference.innerText.substring(reference.innerText.length - 1) ===
              "|"
            ) {
              reference.innerText = reference.innerText.substring(
                0,
                reference.innerText.length - 1
              )
            } else if (
              reference.innerText.substring(reference.innerText.length - 1) !==
              "|"
    ...
    sentence.split("").forEach((letter, index) => {
      ...
      this.timeout = setTimeout(() => {
        ...
        if (nextLetter === " " && self.refs && self.refs[reference]) {
          // Handling space
          return (self.refs[reference].innerHTML =
            self.refs[reference].innerHTML.substring(
              0,
              self.refs[reference].innerHTML.length - 1
            ) + " |")
        } else if (self.refs && self.refs[reference]) {
          // adding next digit
          return (self.refs[reference].innerText =
            self.refs[reference].innerText.substring(
              0,
              self.refs[reference].innerText.length - 1
            ) +
            nextLetter +
            "|")
        }
